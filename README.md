...
| Path | Purpose |
|------|---------|
| `/etc/nginx/conf.d/<domain>.conf` | The site's server config (HTTP→HTTPS + HTTPS proxy). |
...
| `listen ... http2 directive is deprecated` | The config was generated by an older script version. Re-run the current script; it migrates configs carrying its `# HTTPS reverse proxy` marker and backs them up first. |
